...
class QueueWorkerTracker implements TrackerInterface
{
     ...
     * @var Carbon
     */
    protected $lastLoopEventExecutedAt;
    ...
    {
        if (is_null($this->lastLoopEventExecutedAt)) {
            $this->lastLoopEventExecutedAt = now();
        }

        if ($this->lastLoopEventExecutedAt->diffInSeconds() < $this->pingInterval) {
            return;
        }
